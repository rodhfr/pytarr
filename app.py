...
def process_file(file_path, converted_files):
    ...
    if file_ext.lower() in ['.mkv', '.mp4'] and not file_name.endswith(' - 720p'):
        ...
        if not os.path.exists(output_path):
            subprocess.call(['ffmpeg', '-i', file_path, '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-crf', '24', '-x264-params', 'profile=high', '-map', '0', '-c:a', 'copy', '-c:s', 'copy', '-preset', 'veryfast', '-tune', 'fastdecode', output_path])
